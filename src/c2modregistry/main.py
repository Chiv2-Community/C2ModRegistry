...
DEFAULT_MOD_LIST_INDEX_PATH = "./package_db/mod_list_index.txt"
DEFAULT_PACKAGES_PATH = "./package_db/packages"
...
def init(org: str, repoName: str) -> None:
    ...
    if not os.path.exists(f"{DEFAULT_PACKAGES_PATH}/{org}"):
        os.mkdir(f"{DEFAULT_PACKAGES_PATH}/{org}")

    with open(f"{DEFAULT_PACKAGES_PATH}/{org}/{repoName}.json", "w") as file:
...
def add_release(org: str, repoName: str, release_tag: str) -> None:
    ...
    mod = load_mod(org, repoName, DEFAULT_PACKAGES_PATH)
...
def remove_mod(org: str, repoName: str) -> None:
    ...
    os.remove(f"{DEFAULT_PACKAGES_PATH}/{org}/{repoName}.json")

    # If org directory is empty, remove it
    if not os.listdir(f"{DEFAULT_PACKAGES_PATH}/{org}"):
        print(f"Removing empty org {org}...")
        os.rmdir(f"{DEFAULT_PACKAGES_PATH}/{org}")
